...
#ifdef ANDROID
extern int check_paused();
#endif

#define	FRAME_OFFSET(x,y)	(((y)<<10)+(x))

// misc globals
unsigned short bgr2rgb[65536];
int            iResX = 320;
int            iResY = 240;
long           lLowerpart;
...
void DoBufferSwap(void)
{
	...
	signed short h0, x0, y0, w0;
  unsigned short* dest_screen16;
  unsigned short* src_screen16;
  ...
  x0 = PSXDisplay.DisplayPosition.x;
	...
	w0 = PSXDisplay.DisplayMode.x;
  h0 = PreviousPSXDisplay.DisplayMode.y;	
  ...
  dest_screen16 = gp2x_screen16;
  src_screen16  = &psxVuw[FRAME_OFFSET(x0,y0)];

#if !defined(PANDORA)
  if(h0==256)
  {
  	h0 = 240;
  	src_screen16  += ((256-240)>>1)*1024;
  }
#endif
...
#else
  {
    ...
    /* Blit span */
  	GPU_Blitter(src_screen16,	dest_screen16, h0);
  }
  
  if(displayFrameInfo)
  {
     gp2x_printf(NULL, 0, 0, "%s ", szDispBuf);
  }
