...
void CALLBACK GPU_displayText(char * pText)             // some debug func
{
...
void CALLBACK GPU_displayFlags(unsigned long dwFlags)   // some info func
{
 dwCoreFlags=dwFlags;
 BuildDispMenu(0);
}
...
long CALLBACK GPU_init()                                // GPU INIT
{
...
#ifdef _WINDOWS
long CALLBACK GPU_open(HWND hwndGPU)                    // GPU OPEN
{
...
long CALLBACK GPU_close()                               // GPU CLOSE
{
#ifdef _WINDOWS
...
long CALLBACK GPU_shutdown()                            // GPU SHUTDOWN
{
 ...
 if(dwActFixes&32)                                     // pc fps calculation fix
  {
   ...
   if(UseFrameSkip || displayFrameInfo)  
    PCcalcfps();         
  }

  if(displayFrameInfo) //ulKeybits&KEY_SHOWFPS)                             // make fps display buf
  {
...
void CALLBACK GPU_cursor(int iPlayer,int x,int y)
{
 if(iPlayer<0) return;
 if(iPlayer>7) return;
...
void CALLBACK GPU_setMode(unsigned long gdata)
{
...
long CALLBACK GPU_getMode(void)
{
 long iT=0;
...
void GPU_getScreenPic(unsigned char * pMem)
{
/*
...
void CALLBACK GPU_showScreenPic(unsigned char * pMem)
{
...
void CALLBACK GPU_setfix(uint32_t dwFixBits)
{
 dwEmuFixes=dwFixBits;
}

void CALLBACK GPU_vBlank( int val )
{
    vBlank = val;
}
