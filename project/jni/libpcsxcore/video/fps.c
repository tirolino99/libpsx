...
void CheckFrameRate(void)
{
 if(UseFrameSkip)                                      // skipping mode?
 ...
 else                                                  // non-skipping mode:
  {
   ...
   if(displayFrameInfo) calcfps();                // -> and calc fps display
  }
}

#define TIMEBASE 100000

unsigned long gpuTimeGetTime()
{
 struct timeval tv;
...
void FrameSkip(void)
{
 ...
 if(iNumSkips)                                         // we are in skipping mode?
  ...
  {                                                    // we check now, if some limitation is needed, or a new skipping has to get started
   ...
   if(bInitCap || bSkipNextFrame)                      // first time or we skipped before?
    {
     if(UseFrameLimit && !bInitCap)                    // frame limit wanted and not first time called?
      {
       ...
       curticks = gpuTimeGetTime();                       // -> now we calc the time of the last drawn frame + the time we spent skipping
     ...
     lastticks = gpuTimeGetTime();                        // -> we store the start time of the next frame
   ...
   curticks = gpuTimeGetTime();                           // get the current time (we are now at the end of one drawn frame)
   _ticks_since_last_update = curticks - lastticks;
   ...
   if (overslept < 0)
	overslept = 0;
   lastticks = gpuTimeGetTime();                          // ok, start time of the next frame
  }
