  ...
  while(!bEndThread)                                    // until we are shutting down
  {
   ...
   if(dwNewChannel)                                    // new channel should start immedately?
    ...
    if(!iUseTimer) // && !iSecureStart)
    {
      ...
      {
#ifdef ANDROID
        const int waitlength = (int)(((float)NSSIZE/44.1) * 1000.0); // * ((float)(BIAS_CYCLE > 4 ? ((float)(BIAS_CYCLE)/4.0) : 1.0)));
#else
        const int waitlength = ((NSSIZE/45) * 1000);
#endif
        tick_counter = 0;
        if(last_render_ticks == 0)
        {
          gettimeofday(&current_time, NULL);
          last_render_ticks = (current_time.tv_sec * 1000000 + current_time.tv_usec);
        }
        else
        {
          gettimeofday(&current_time, NULL);
          render_ticks = (current_time.tv_sec * 1000000 + current_time.tv_usec);
          if(render_ticks < last_render_ticks + waitlength)
          {
            s32 render_delta = (last_render_ticks + waitlength) - render_ticks;
            if(render_delta > 0 && render_delta <= waitlength)
            {
              usleep(render_delta);
#if 0
              s32 irqchecks = (render_delta / 100) + 1;
              while(irqchecks-- > 0)
              {
              ...
              if(iSpuAsyncWait)
              {
                __android_log_print(ANDROID_LOG_WARN, LOG_TAG, "render_delta %d", render_delta);
              }
...
ENDX:   ;
  ...
  //if(++iCycle >= 5)    
#else
    if(++iCycle >= (int)((44.1 * 20.0)/(float)NSSIZE))
#endif
  ...
  //  if(iCycle > 44000/1000*LATENCY + 100*LATENCY/1000)
    {
      ...
      pS = &pSpuBuffer[0]; // Use this to read memory address to fix Android compiler bug affecting Diablo sound
      iCycle = 0;
    }

#if 0    
   if(bIRQReturn && iUseTimer!=2)
   { 
     //DWORD dwWatchTime=timeGetTime_spu()+5000;
      bIRQReturn=0;
      if(iSpuAsyncWait) 
        iSpuAsyncWait = 1;
...
#ifdef ANDROID
  // let SoundFeedStreamData/render_audio finish
  //usleep(100000);
#endif
...
long cpu_cycles = 0;
void CALLBACK SPU_async(uint32_t cycle)
{
	//cpu_cycles += cycle;
 ...
 if(iSpuAsyncWait)
  {
   iSpuAsyncWait++;
   if(iSpuAsyncWait<=5000*BIAS_CYCLE) return;
   iSpuAsyncWait=0;
...
void SetupTimer(void)
{
 ...
 pS = &pSpuBuffer[0];                               // setup soundbuffer pointer
...
{ 
 ...
 pSpuBuffer=(short *)malloc(44100*2);            // alloc mixing buffer
...
void CALLBACK SPU_registerCallback(void (CALLBACK *callback)(void))
{
